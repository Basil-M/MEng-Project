    ...
    with open(objfile, "w") as fout:
        for file in os.listdir(dir):
            if re.match(regstr, file):
                with open(os.path.join(dir, file)) as filein:
                    fout.write(filein.read())


def JoinFiles(fnx, fny, ofn):
    with open(fnx) as fin:
        lx = [vv for vv in fin.read().split('\n') if vv != ""]
    with open(fny) as fin:
...
def SplitTables(fn, limit=3):
    ...
    for ii in rst:
        print('%d columns' % ii)
        with open(fn.replace('.txt', '') + '.split.%d.txt' % ii, 'w') as fout:
            with open(fn) as fin:
                for line in fin:
                    lln = line.rstrip('\r\n').split('\t')
                    if len(lln) == ii:
...
def GetLines(fn):
    with open(fn, errors='ignore') as fin:
